...
def longest_streak(string)
  string.each_char.slice_when(&:!=).map(&:join).reverse.max_by(&:length)
  # slice when char a != char b, join them back; since we want last, do reverse; get the max of length
end

#   streaking = false
#   hash = Hash.new {|k,v| hash[k] = []}
#   string.chars.each_cons(2).each do |(a, b)| 
#     temp = 0
#     temp += 1 if 
#   end
...
# longest_streak('aaaxyyyyyzzaaaa') # => 'yyyyy
